...
function printLengthIfString(input: unknown) {
    if (isString(input)) {
...
function doubleIfNumber(value: any): any {
    if (isNumber(value)) {
        return value * 2;
    }
    return value;
...
type Dice = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9;

function rollDeice(value: number) {
...
function isDiceWithReturnTypeDice(value: number): value is Dice {
...
function isDiceWithReturnTrue(value: number): value is Dice {
    return true;
}
